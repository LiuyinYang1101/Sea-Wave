...
def cosine_loss(predictions, targets):
    ...
    return 0-cos(predictions.squeeze(), targets.squeeze()).mean()

def pearson_correlation(x, y, axis=-1, eps=0.00001, nan_value=0):
    mean_fn = lambda x: torch.mean(x, axis=axis, keepdim=True)
    std_fn = lambda x: torch.sum(x ** 2, axis=axis, keepdim=True)
    x_mean, y_mean = mean_fn(x), mean_fn(y)
    x_std, y_std = std_fn(x - x_mean), std_fn(y - y_mean)
    numerator = torch.sum((x - x_mean) * (y - y_mean), axis=axis, keepdim=True) 
    denominator = torch.sqrt(x_std * y_std)
    pearson_correlation = numerator / torch.clip(denominator, eps)
    return torch.nan_to_num(pearson_correlation, nan_value)

def pearson_loss(x, y, axis=-1):
    #print(torch.isnan(x).any(), torch.isnan(y).any())
    pearson_corr = pearson_correlation(x, y, axis)
    return -torch.mean(pearson_corr)
